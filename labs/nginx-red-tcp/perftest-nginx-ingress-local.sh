...
echo "single request ..."
curl -s -H 'Host: internal.example.com' -w "\nTime: %{time_total}s\nSpeed: %{speed_download} bytes/s\n" -o /dev/null http://$NODEIP:$NODEPORT/test/512kb

echo ""
echo "Sending $CONCURRENT_REQUESTS for $DURATION ..."
wrk -H 'Host: internal.example.com' -t$THREADS -c$CONCURRENT_REQUESTS -d$DURATION --latency http://$NODEIP:$NODEPORT/test/512kb
